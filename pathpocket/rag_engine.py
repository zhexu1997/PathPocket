"""
PathPocket Engine Module
Self-contained RAG engine functionality
"""
...
from pathpocket.lightrag_exceptions import PipelineCancelledException
...
from pathpocket.lightrag_utils import get_env_value, resolve_query_global_config

from pathpocket.lightrag_kg import (
    STORAGES,
...
from pathpocket.lightrag_constants import GRAPH_FIELD_SEP
...
from pathpocket.lightrag_kg.types import KnowledgeGraph
...
class LightRAG:
    ...
    embedding_token_limit: Optional[int] = field(default=None, init=False)
    ...
    rerank_pairs_batch_func: Callable[..., object] | None = field(default=None)
    """Score many (query, document) pairs in one GPU batch; used for per-option MCQ rerank."""

    min_rerank_score: float = field(
        default=get_env_value("MIN_RERANK_SCORE", DEFAULT_MIN_RERANK_SCORE, float)
    )
    """Default minimum rerank score; used when type-specific thresholds are unset."""

    min_rerank_score_entities: Optional[float] = field(default=None)
    """Minimum rerank score for entity evidence; defaults to ``min_rerank_score``. Env: MIN_RERANK_SCORE_ENTITIES."""

    min_rerank_score_relations: Optional[float] = field(default=None)
    """Minimum rerank score for relation evidence; defaults to ``min_rerank_score``. Env: MIN_RERANK_SCORE_RELATIONS."""

    min_rerank_score_chunks: Optional[float] = field(default=None)
    """Minimum rerank score for chunk evidence; defaults to ``min_rerank_score``. Env: MIN_RERANK_SCORE_CHUNKS."""

    min_rerank_score_similar_images: Optional[float] = field(default=None)
    """Minimum rerank score for similar-image caption rerank; defaults to ``min_rerank_score``. Env: MIN_RERANK_SCORE_SIMILAR_IMAGES."""

    relation_entity_iou_threshold: float = field(
        default=get_env_value(
    ...
    """Minimum entity Jaccard IoU for keeping retrieved relations before rerank (mix/local/global paths)."""

    enable_site_file_path_rerank: bool = field(
        default_factory=lambda: os.getenv(
            "ENABLE_SITE_FILE_PATH_RERANK", "true"
        ).lower()
        == "true"
    )
    """If True, drop KG/chunk evidence whose file_path scores low on alignment with structured ``site`` (rerank)."""

    min_site_file_path_rerank_score: Optional[float] = field(default=None)
    """Minimum site–file_path rerank score; None means use ``min_rerank_score``."""

    evidence_level_title_category_json: Optional[str] = field(
        default_factory=lambda: (
            os.getenv("EVIDENCE_LEVEL_TITLE_CATEGORY_JSON", "").strip() or None
        )
    )
    """If set, map ``file_path`` on retrieved chunks/entities/relations to catalog evidence tiers (1–8)."""
    ...
    cosine_better_than_threshold: float = field(
        default_factory=lambda: float(
            os.getenv("COSINE_BETTER_THAN_THRESHOLD", "")
            or os.getenv("COSINE_THRESHOLD", "0.2")
        )
    )

    relation_cosine_better_than_threshold: Optional[float] = field(default=None)
    """Cosine threshold for relationship vector store; defaults to ``cosine_better_than_threshold``. Env: RELATION_COSINE_BETTER_THAN_THRESHOLD."""

    chunk_cosine_better_than_threshold: Optional[float] = field(default=None)
    """Cosine threshold for chunk vector store; defaults to ``cosine_better_than_threshold``. Env: CHUNK_COSINE_BETTER_THAN_THRESHOLD."""

    pathology_images_cosine_better_than_threshold: Optional[float] = field(default=None)
    """Cosine threshold for pathology_images (Virchow2) vector store; defaults to ``cosine_better_than_threshold``.
    Env: PATHOLOGY_IMAGES_COSINE_BETTER_THAN_THRESHOLD."""
    ...
    def __post_init__(self):
        ...
        resolve_query_global_config(global_config)
        self.pathology_images_cosine_better_than_threshold = global_config[
            "pathology_images_cosine_better_than_threshold"
        ]
        self.min_rerank_score_similar_images = global_config[
        ...
        # Pathology image feature storage (Virchow2 features)
        # Only create if conch_feature_func is provided in addon_params
        logger.debug(f"Checking addon_params for conch_feature_func...")
        logger.debug(f"addon_params type: {type(self.addon_params)}")
        logger.debug(f"addon_params keys: {list(self.addon_params.keys()) if isinstance(self.addon_params, dict) else 'not a dict'}")
        conch_feature_func = self.addon_params.get("conch_feature_func") if isinstance(self.addon_params, dict) else None
        if conch_feature_func:
            # Use the same vector storage type as the main vector storage (PGVectorStorage or SimpleVectorStorage)
            vector_storage_type = self.__dict__.get("vector_storage", "SimpleVectorStorage")
            logger.info(f"Initializing pathology_images_vdb storage (Virchow2 features) using {vector_storage_type}")
            logger.debug(f"conch_feature_func type: {type(conch_feature_func)}")
            ...
            if vector_storage_type == "PGVectorStorage":
                ...
                self.pathology_images_vdb: Optional[BaseVectorStorage] = PGVectorStorage(
                ...
                logger.info(f"pathology_images_vdb initialized as PGVectorStorage (will access VDB_IMAGES table in PostgreSQL)")
            else:
                ...
                _img_vdb_cos = resolve_vector_store_cosine_threshold(
                    "pathology_images", self.__dict__
                )
                self.pathology_images_vdb: Optional[BaseVectorStorage] = SimpleVectorStorage(
                    namespace="pathology_images",
                    workspace=self.workspace,
                    global_config=self.__dict__,
                    embedding_func=conch_feature_func,
                ...
                # Get the file path where it will be saved (os is already imported at top of file)
                working_dir = self.__dict__.get("working_dir", "./rag_storage")
                workspace_dir = os.path.join(working_dir, self.workspace) if self.workspace else working_dir
                vdb_file_path = os.path.join(workspace_dir, "vdb_pathology_images.json")
                logger.info(f"pathology_images_vdb initialized as SimpleVectorStorage (will be saved to: {vdb_file_path})")
        else:
            self.pathology_images_vdb: Optional[BaseVectorStorage] = None
            logger.warning("pathology_images_vdb not initialized (conch_feature_func not found in addon_params)")
            logger.debug(f"addon_params type: {type(self.addon_params)}")
            logger.debug(f"addon_params keys: {list(self.addon_params.keys()) if isinstance(self.addon_params, dict) else 'not a dict'}")
            logger.debug(f"addon_params content: {self.addon_params}")
    ...
    async def initialize_storages(self):
        ...
        if self._storages_status == StoragesStatus.CREATED:
            ...
            from pathpocket.shared_storage import initialize_pipeline_status
    ...
    async def finalize_storages(self):
        ...
        if self._storages_status == StoragesStatus.INITIALIZED:
            storages = [
                ...
                ("pathology_images_vdb", self.pathology_images_vdb),
    ...
    async def _migrate_entity_relation_data(self, processed_docs: dict):
        ...
        for edge in all_edges:
            ...
            # Check if this is a hyperedge (multi-entity relation)
            entities = None
            if "entities" in edge:
                # Extract entities from hyperedge (format: "entity1<SEP>entity2<SEP>...")
                entities_str = edge["entities"]
                if isinstance(entities_str, str):
                    entities = [e.strip() for e in entities_str.split(GRAPH_FIELD_SEP) if e.strip()]
                elif isinstance(entities_str, list):
                    entities = entities_str
            elif "source" in edge and "target" in edge:
                # Binary relation (legacy format)
                src = edge.get("source")
                tgt = edge.get("target")
                if src and tgt:
                    entities = [src, tgt]
            ...
            # Generate all possible entity pairs from the relation
            # For n-ary relations, extract all pairs (combinations)
            import itertools
            ...
            for chunk_id in source_ids:
                ...
                if doc_id:
                    ...
                    # Add all entity pairs from this relation
                    for pair in entity_pairs:
                        # Use tuple for set operations, convert to list later
                        doc_relations[doc_id].add(tuple(sorted(pair)))
    ...
    def _get_storage_class(self, storage_name: str) -> Callable[..., Any]:
        ...
        if storage_name == "JsonKVStorage":
            from pathpocket.lightrag_kg.json_kv_impl import JsonKVStorage

            return JsonKVStorage
        elif storage_name == "NanoVectorDBStorage":
            from pathpocket.lightrag_kg.nano_vector_db_impl import NanoVectorDBStorage

            return NanoVectorDBStorage
        elif storage_name == "NetworkXStorage":
            from pathpocket.lightrag_kg.networkx_impl import NetworkXStorage

            return NetworkXStorage
        elif storage_name == "HyperNetXStorage":
            from pathpocket.lightrag_kg.hypernetx_impl import HyperNetXStorage

            return HyperNetXStorage
        elif storage_name == "JsonDocStatusStorage":
            from pathpocket.lightrag_kg.json_doc_status_impl import JsonDocStatusStorage
    ...
    async def ainsert_custom_chunks(
        self, full_text: str, text_chunks: list[str], doc_id: Optional[str] = None
    ) -> None:
    ...
    async def aquery_data(
        ...
        Returns:
            ...
            **Success Response:**
            ```python
            {
                ...
                "data": {
                    ...
                    "relationships": [
                        {
                            "entities": list,        # Hyperedge entity names (preferred over src_id/tgt_id)
                            "src_id": str,           # Source entity name (binary fallback)
                            "tgt_id": str,           # Target entity name (binary fallback)
                            ...
                            "created_at": str,       # Creation timestamp
                            "reference_id": str,     # Reference identifier for citations
                            "entity_iou": float,     # Optional: Jaccard vs retrieval entity set
                            "rerank_score": float,   # Optional: cross-encoder relevance
                            "rerank_query": str,     # Optional: query string that yielded rerank_score (MCQ: best option)
                            "rerank_queries": list,  # Optional: all rerank query strings used for this pass
                        }
                    ],
                    "chunks": [
                        {
                            ...
                            "rerank_query": str,     # Optional
                            "rerank_queries": list,  # Optional
                        }
                    ],
                    "references": [
                        {
                ...
                "metadata": {
                    ...
                    "structured_retrieval": Optional[dict],  # pathology_structured_llm + mix/rerank/chunk query fields
                    "processing_info": {
            ...
            **Query Mode Differences:**
            - **local**: Focuses on entities and their related chunks from structured retrieval
            - **global**: Focuses on relationships and their connected entities from structured retrieval
            ...
            **Common Failure Cases:**
            - Empty query string
            - Structured retrieval produced no usable entities, relations, or chunks
        ...
        data_param = QueryParam(
            ...
            mix_structured_retrieval=param.mix_structured_retrieval,
            mix_structured_chunk_rerank_done=param.mix_structured_chunk_rerank_done,
            precomputed_keywords_dict=param.precomputed_keywords_dict,
        )
    ...
    async def aquery_llm(
        ...
        try:
            ...
            if query_result is None:
                allow_empty_context_llm = (
                    os.getenv("ALLOW_EMPTY_CONTEXT_LLM", "false").lower() == "true"
                )
                if allow_empty_context_llm and param.mode != "bypass":
                    # Even if retrieval found no usable context, still run an LLM generation
                    # with an empty context payload. This keeps the original mode while
                    # avoiding a fixed fail_response string.
                    use_llm_func = param.model_func or global_config["llm_model_func"]
                    use_llm_func = partial(use_llm_func, _priority=8)

                    response_type = (
                        param.response_type
                        if getattr(param, "response_type", None)
                        else "Multiple Paragraphs"
                    )
                    user_prompt = (
                        f"\n\n{param.user_prompt}"
                        if getattr(param, "user_prompt", None)
                        else "n/a"
                    )
                    sys_prompt_temp = (
                        system_prompt if system_prompt else PROMPTS["rag_response"]
                    )
                    ...
                    if type(response) is str:
                        return {
                            "status": "success",
                            "message": "LLM generated with empty retrieval context",
                            "data": {},
                            "metadata": {
                                "failure_reason": "no_results",
                                "mode": param.mode,
                                "empty_context_llm": True,
                            },
                            "llm_response": {
                                "content": response,
                                "response_iterator": None,
                        ...
                        return {
                            "status": "success",
                            "message": "LLM streaming with empty retrieval context",
                            "data": {},
                            "metadata": {
                                "failure_reason": "no_results",
                                "mode": param.mode,
        ...
        **Concurrency Control Design:**
        ...
        Returns:
            DeletionResult: An object containing the outcome of the deletion process.
                ...
                - `file_path` (Optional[str]): The file path of the deleted document, if available.
        """
    ...
    async def aexport_data(
        ...
        """
        from pathpocket.lightrag_utils import aexport_data as utils_aexport_data
...
# Export as PathPocketEngine
PathPocketEngine = LightRAG
__all__ = ["PathPocketEngine", "LightRAG"]

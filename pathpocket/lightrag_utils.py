...
if TYPE_CHECKING:
    from pathpocket.lightrag_base import BaseKVStorage, BaseVectorStorage, QueryParam
...
class EmbeddingFunc:
    ...
    embedding_dim: int
    func: callable
    max_token_size: Optional[int] = None  # Token limit for the embedding model
...
def compute_args_hash(*args: Any) -> str:
    ...
    Returns:
        str: Hash string
    """
    # Join with separator so different (mode, query, top_k, ...) cannot collide
    # e.g. ("a","12","3") vs ("a1","2","3") both become distinct strings
    args_str = "\x00".join([str(arg) for arg in args])
...
_STAGE3_MERGE_PLACEHOLDER = (
    "Descriptions merged without LLM summarization in Stage 3"
)


def scrub_stage3_merge_placeholder(text: Any) -> str:
    """Remove Stage-3 indexing boilerplate from text sent to the answer LLM."""
    if text is None:
        return ""
    s = str(text).strip()
    if not s or s == "UNKNOWN":
        return ""
    if _STAGE3_MERGE_PLACEHOLDER not in s:
        return s
    if GRAPH_FIELD_SEP in s:
        parts = [
            p.strip()
            for p in s.split(GRAPH_FIELD_SEP)
            if p.strip() and p.strip() != _STAGE3_MERGE_PLACEHOLDER
        ]
        s = GRAPH_FIELD_SEP.join(parts)
    else:
        s = s.replace(_STAGE3_MERGE_PLACEHOLDER, "").strip()
    s = re.sub(r"[ \t]{2,}", " ", s)
    s = re.sub(r"\n{3,}", "\n\n", s)
    return s.strip()
...
def check_storage_env_vars(storage_name: str) -> None:
    ...
    """
    from pathpocket.lightrag_kg import STORAGE_ENV_REQUIREMENTS
...
def _usable_evidence_file_path(item: dict) -> str | None:
    """Return a non-empty file_path for rerank, or None if unknown / missing."""
    ...
    if not s:
        return None
    low = s.lower()
    if low == "unknown_source" or low == "unknown":
        return None
    return s


def cfg_float(config: dict, key: str, default: float) -> float:
    """Read a float from a config dict; missing or None values use *default*."""
...
def resolve_query_global_config(global_config: dict) -> dict:
    """
    Resolve Optional threshold fields on global_config from asdict(self).

    Dataclass Optional fields and nested .get(key, default) can leave None values
    that break float(None) during query/metadata assembly.
    """
    ...
    cos_val = global_config.get("cosine_better_than_threshold")
    if cos_val is None:
        cos_val = kwargs.get("cosine_better_than_threshold")
    base_cos = float(cos_val) if cos_val is not None else float(DEFAULT_COSINE_THRESHOLD)
    kwargs.setdefault("cosine_better_than_threshold", base_cos)

    rel_cos = global_config.get("relation_cosine_better_than_threshold")
    if rel_cos is None:
        rel_cos = _env_float("RELATION_COSINE_BETTER_THAN_THRESHOLD")
    global_config["relation_cosine_better_than_threshold"] = (
        float(rel_cos) if rel_cos is not None else base_cos
    )

    chunk_cos = global_config.get("chunk_cosine_better_than_threshold")
    if chunk_cos is None:
        chunk_cos = _env_float("CHUNK_COSINE_BETTER_THAN_THRESHOLD")
    global_config["chunk_cosine_better_than_threshold"] = (
        float(chunk_cos) if chunk_cos is not None else base_cos
    )

    img_cos = global_config.get("pathology_images_cosine_better_than_threshold")
    if img_cos is None:
        img_cos = _env_float("IMAGE_SIMILARITY_THRESHOLD")
    if img_cos is None:
        img_cos = _env_float("PATHOLOGY_IMAGES_COSINE_BETTER_THAN_THRESHOLD")
    global_config["pathology_images_cosine_better_than_threshold"] = (
        float(img_cos) if img_cos is not None else base_cos
    )

    base_rr = cfg_float(global_config, "min_rerank_score", DEFAULT_MIN_RERANK_SCORE)
    global_config["min_rerank_score"] = base_rr

    ent_rr = global_config.get("min_rerank_score_entities")
    if ent_rr is None:
        ent_rr = _env_float("MIN_RERANK_SCORE_ENTITIES")
    global_config["min_rerank_score_entities"] = (
        float(ent_rr) if ent_rr is not None else base_rr
    )

    rel_rr = global_config.get("min_rerank_score_relations")
    if rel_rr is None:
        rel_rr = _env_float("MIN_RERANK_SCORE_RELATIONS")
    global_config["min_rerank_score_relations"] = (
        float(rel_rr) if rel_rr is not None else base_rr
    )

    chunk_rr = global_config.get("min_rerank_score_chunks")
    if chunk_rr is None:
        chunk_rr = _env_float("MIN_RERANK_SCORE_CHUNKS")
    global_config["min_rerank_score_chunks"] = (
        float(chunk_rr) if chunk_rr is not None else base_rr
    )

    sim_img_rr = global_config.get("min_rerank_score_similar_images")
    if sim_img_rr is None:
        sim_img_rr = _env_float("MIN_RERANK_SCORE_SIMILAR_IMAGES")
    global_config["min_rerank_score_similar_images"] = (
        float(sim_img_rr) if sim_img_rr is not None else base_rr
    )

    iou = global_config.get("relation_entity_iou_threshold")
    if iou is None:
        iou = _env_float("RELATION_ENTITY_IOU_THRESHOLD")
    global_config["relation_entity_iou_threshold"] = (
        float(iou) if iou is not None else float(DEFAULT_RELATION_ENTITY_IOU_THRESHOLD)
    )

    return global_config


def _site_file_path_rerank_query(site: str) -> str:
    return (
        f"病变/取材部位：{site}\n\n"
        "下列文本是一条证据对应的文档存储路径（可含目录、文件名或病历标识等）。"
        "请判断该路径所指向的资料是否很可能与上述部位相关（路径或文件名中是否出现相同或相近解剖部位、亚专科或器官关键词），"
        "而不是明显来自与题干部位不相关的其他器官或系统的文档。"
    )


def _site_file_path_rerank_enabled(global_config: dict, enable_rerank: bool) -> bool:
    if not enable_rerank:
        return False
    v = global_config.get("enable_site_file_path_rerank")
    if v is not None:
        return bool(v)
    return os.getenv("ENABLE_SITE_FILE_PATH_RERANK", "true").lower() == "true"


def _min_site_file_path_rerank_score(global_config: dict) -> float:
    v = global_config.get("min_site_file_path_rerank_score")
    if v is not None:
        try:
            return float(v)
        except (TypeError, ValueError):
            pass
    env = os.getenv("MIN_SITE_FILE_PATH_RERANK_SCORE", "").strip()
    if env:
        return float(env)
    return cfg_float(global_config, "min_rerank_score", DEFAULT_MIN_RERANK_SCORE)


async def filter_evidence_by_site_file_path_rerank(
    *,
    site: str,
    items: list[dict],
    global_config: dict,
    ...
    """
    Drop KG/context items whose file_path is judged unrelated to ``site`` by the rerank model.

    Items without a usable ``file_path`` are kept unchanged (no score).
    """
    site = (site or "").strip()
    if not site or not items:
        return items

    if not _site_file_path_rerank_enabled(global_config, enable_rerank):
        return items

    rerank_func = global_config.get("rerank_model_func")
    if not rerank_func:
        logger.warning(
            "Site–file_path rerank is enabled but rerank_model_func is missing; skip (%s)",
            item_label,
        )
        return items

    min_score = _min_site_file_path_rerank_score(global_config)
    pending: list[tuple[int, str]] = []
    for i, item in enumerate(items):
        fp = _usable_evidence_file_path(item)
        if fp is None:
            continue
        pending.append((i, fp))
    ...
    query = _site_file_path_rerank_query(site)
    documents = [f"文档路径：{fp}" for _, fp in pending]
    ...
    try:
        ...
        logger.error("Site–file_path rerank failed (%s): %s", item_label, e)
        return items

    if not rerank_results or not isinstance(rerank_results[0], dict):
        logger.warning("Site–file_path rerank: empty or legacy format (%s)", item_label)
        return items
    if "index" not in rerank_results[0]:
        logger.warning("Site–file_path rerank: missing index field (%s)", item_label)
        return items

    scores_by_pidx: dict[int, float] = {}
    for result in rerank_results:
        pidx = int(result["index"])
        if 0 <= pidx < len(pending):
            scores_by_pidx[pidx] = float(result.get("relevance_score", 0.0))

    orig_to_pidx = {orig_i: pidx for pidx, (orig_i, _) in enumerate(pending)}

    kept: list[dict] = []
    dropped = 0
    for i, item in enumerate(items):
        if _usable_evidence_file_path(item) is None:
            kept.append(item)
            continue
        pidx = orig_to_pidx[i]
        score = scores_by_pidx.get(pidx, 0.0)
        item2 = item.copy()
        item2["site_file_path_rerank_score"] = score
        item2["site_file_path_rerank_query"] = query
        if score >= min_score:
            kept.append(item2)
        else:
            dropped += 1
    ...
    if dropped:
        logger.info(
            "Site–file_path rerank filter (%s): kept %d / %d (min_score=%.4f, site=%r)",
            item_label,
            len(kept),
            len(items),
            min_score,
            site[:80] + ("…" if len(site) > 80 else ""),
        )
    return kept
            ...
            if isinstance(rerank_results[0], dict) and "index" in rerank_results[0]:
                ...
                for result in rerank_results:
                    ...
                    if 0 <= index < len(retrieved_docs):
                        ...
                        doc["rerank_query"] = query
                        doc["rerank_queries"] = [query] if (query or "").strip() else []
...
_CHUNK_PATH_FILE_EXT = (
    r"(?:jpg|jpeg|png|gif|bmp|webp|tiff|tif|pdf|xlsx?|csv|json|txt|md|svg)"
)
# ``(Path: /.../Title (5th Edition)/.../hash.jpg)`` — inner ``)`` breaks ``[^)]+``;
# anchor removal at ``.<ext>)`` instead.
_CHUNK_PATH_IN_PARENS_RE = re.compile(
    rf"""\(\s*Path\s*:\s*.+?\.{_CHUNK_PATH_FILE_EXT}\s*\)""",
    re.IGNORECASE | re.DOTALL,
)
_CHUNK_IMAGE_TABLE_PATH_PREFIX_RE = re.compile(
    rf"""Image\s+Content\s*\(\s*Path\s*:\s*.+?\.{_CHUNK_PATH_FILE_EXT}\s*\)\s*[：:]\s*\n?""",
    re.IGNORECASE | re.DOTALL,
)
_CHUNK_TABLE_PATH_PREFIX_RE = re.compile(
    rf"""Table\s+Content\s*\(\s*Path\s*:\s*.+?\.{_CHUNK_PATH_FILE_EXT}\s*\)\s*[：:]\s*\n?""",
    re.IGNORECASE | re.DOTALL,
)
# Bare Unix-style absolute paths to common roots (spaces/parens allowed in segments).
_CHUNK_BARE_ABS_PATH_RE = re.compile(
    rf"(?<![\w/])(?:/media|/home|/Users|/mnt|/opt|/var|/tmp|/project|/data|/workspace)(?:/[\w\- .()+%,｜\[\]]+)+\.{_CHUNK_PATH_FILE_EXT}\b",
    re.IGNORECASE,
)
# Windows: ``C:\...`` or ``D:/...``
_CHUNK_WIN_ABS_PATH_RE = re.compile(
    rf"""(?<![\w])[A-Za-z]:\\(?:[^\\:\n]+\\)*[^\\:\n]+\.{_CHUNK_PATH_FILE_EXT}\b""",
    re.IGNORECASE,
)
_CHUNK_WIN_ABS_PATH_SLASH_RE = re.compile(
    rf"""(?<![\w])[A-Za-z]:/(?:[^/:\n]+/)+[^/:\n]+\.{_CHUNK_PATH_FILE_EXT}\b""",
    re.IGNORECASE,
)


def strip_chunk_local_path_hints_for_llm(text: str) -> str:
    """
    Remove local filesystem path hints embedded in chunk text (e.g. Mineru-style
    ``(Path: /abs/path/to/file.jpg)``) so they are not included in the LLM context.

    Handles paths whose directory names contain parentheses (e.g. ``(5th Edition)``)
    by matching through a file extension before the closing ``)``.

    Also strips common bare absolute paths (``/media/...``, ``/home/...``, Windows drives).
    """
    if not isinstance(text, str) or not text.strip():
        return text
    t = text
    # Repeat: multiple ``(Path: ...)`` or overlaps
    for _ in range(8):
        n0 = len(t)
        t = _CHUNK_PATH_IN_PARENS_RE.sub("", t)
        t = _CHUNK_IMAGE_TABLE_PATH_PREFIX_RE.sub("", t)
        t = _CHUNK_TABLE_PATH_PREFIX_RE.sub("", t)
        if len(t) == n0:
            break
    # Bare absolute paths (leftover after parenthesis removal or pasted alone)
    for _ in range(4):
        n0 = len(t)
        t = _CHUNK_BARE_ABS_PATH_RE.sub("", t)
        t = _CHUNK_WIN_ABS_PATH_RE.sub("", t)
        t = _CHUNK_WIN_ABS_PATH_SLASH_RE.sub("", t)
        if len(t) == n0:
            break
    # ``(Path: ...)`` without a known extension (rare)
    t = re.sub(r"\(\s*Path\s*:\s*[^\n()]+\)", "", t, flags=re.IGNORECASE)
    t = re.sub(
        r"Image\s+Content\s*\(\s*Path\s*:\s*[^\n()]+\)\s*[：:]\s*\n?",
        "",
        t,
        flags=re.IGNORECASE,
    )
    t = re.sub(
        r"Table\s+Content\s*\(\s*Path\s*:\s*[^\n()]+\)\s*[：:]\s*\n?",
        "",
        t,
        flags=re.IGNORECASE,
    )
...
def dedupe_similar_image_content_caption(record: dict[str, Any]) -> None:
    """
    Similar-image records often carry the same text in both ``content`` and ``caption``.
    Keep a single field: prefer ``content``; remove redundant ``caption`` when it matches
    or when ``content`` is empty (then promote ``caption`` to ``content``).
    """
    if not isinstance(record, dict):
        return
    cap = record.get("caption")
    if cap is None and "caption" not in record:
        return
    c = record.get("content")
    sc = str(c).strip() if c is not None else ""
    scap = str(cap).strip() if cap is not None else ""
    if not scap:
        record.pop("caption", None)
        return
    if sc == scap:
        record.pop("caption", None)
    elif not sc and scap:
        record["content"] = scap
        record.pop("caption", None)


def emit_query_progress(query_param: Any, phase: str) -> None:
    """Notify optional UI callback attached to ``QueryParam.progress_callback``."""
    cb = getattr(query_param, "progress_callback", None)
    if not cb:
        return
    try:
        cb(phase)
...
async def process_chunks_unified(
    ...
    # 0. Site vs file_path alignment (rerank), before content rerank
    rs = (retrieval_site or "").strip()
    if rs:
        unique_chunks = await filter_evidence_by_site_file_path_rerank(
            site=rs,
            items=unique_chunks,
            global_config=global_config,
            enable_rerank=query_param.enable_rerank,
            item_label=f"chunks:{source_type}",
        )
        if not unique_chunks:
            return []

    enrich_evidence_items_from_catalog(unique_chunks, global_config)

    # 1. Apply reranking if enabled and query is provided
    skip_rerank = getattr(query_param, "mix_structured_chunk_rerank_done", False)
    if query_param.enable_rerank and query and unique_chunks and not skip_rerank:
    ...
    if query_param.enable_rerank and unique_chunks:
        _base_rr = cfg_float(global_config, "min_rerank_score", DEFAULT_MIN_RERANK_SCORE)
        min_rerank_score = cfg_float(global_config, "min_rerank_score_chunks", _base_rr)
    ...
    for i, chunk in enumerate(unique_chunks):
        ...
        _c = chunk_with_id.get("content")
        if isinstance(_c, str) and _c:
            chunk_with_id["content"] = strip_chunk_local_path_hints_for_llm(_c)
        final_chunks.append(chunk_with_id)

    return final_chunks


def normalize_source_ids_limit_method(method: Optional[str]) -> str:
...
def _clean_image_entity_description_for_user(entity_dict: dict) -> dict:
    """
    清理图像实体的描述，移除图像路径信息（用于用户格式转换）
    
    Args:
        entity_dict: 实体字典（包含description字段）
    ...
    if is_image_entity and isinstance(description, str):
        # 匹配并移除整个前缀部分
        pattern = r'^Entity:\s*[^\n]+\(image\)\s*\n\s*\nImage\s+Content\s*\(Path:\s*[^)]+\)\s*:\s*\n?'
        cleaned_description = re.sub(pattern, '', description, flags=re.IGNORECASE | re.MULTILINE)
        
        # 如果还有残留的 "Image Content (Path: ...):" 模式
        ...
        entity_dict["description"] = cleaned_description
    
    return entity_dict


def _attach_evidence_catalog_fields(dst: dict, src: dict) -> None:
    """Copy filename-derived catalog fields from retrieval context onto API-facing dicts."""
    if "evidence_level" in src:
        try:
            dst["evidence_level"] = int(src["evidence_level"])
        except (TypeError, ValueError):
            dst["evidence_level"] = src["evidence_level"]
    if "evidence_level_label" in src:
        dst["evidence_level_label"] = src["evidence_level_label"]
    if "evidence_category" in src:
        dst["evidence_category"] = src["evidence_category"]
    if "evidence_catalog_title" in src:
        dst["evidence_catalog_title"] = src["evidence_catalog_title"]
...
def convert_to_user_format(
    ...
    for entity in entities_context:
        ...
        if original_entity:
            ...
            # 清理图像实体的描述
            formatted_entity = _clean_image_entity_description_for_user(formatted_entity)
            if entity.get("rerank_score") is not None:
                try:
                    formatted_entity["rerank_score"] = float(entity["rerank_score"])
                except (TypeError, ValueError):
                    pass
            rq = entity.get("rerank_query")
            if rq is not None and str(rq).strip():
                formatted_entity["rerank_query"] = str(rq).strip()
            rqs = entity.get("rerank_queries")
            if isinstance(rqs, list) and rqs:
                formatted_entity["rerank_queries"] = [
                    str(x) for x in rqs if x is not None and str(x).strip()
                ]
            _attach_evidence_catalog_fields(formatted_entity, entity)
    ...
    # Support both binary relations (src_id/tgt_id) and hyperedges (entities list)
    formatted_relationships = []
    for relation in relations_context:
        # Check if this is a hyperedge (multi-entity relation) or binary relation
        entities = relation.get("entities", [])
        if isinstance(entities, str):
            # If entities is a string, split it
            entities = [e.strip() for e in entities.split(GRAPH_FIELD_SEP) if e.strip()]
        
        # Try to get original data first
        original_relation = None
        if relation_id_to_original:
            # For hyperedges, use sorted entities tuple as key
            if entities and len(entities) > 0:
                relation_key = tuple(sorted(entities))
                if relation_key in relation_id_to_original:
                    original_relation = relation_id_to_original[relation_key]
            # Fallback to entity1/entity2 for binary relations
            if not original_relation:
                entity1 = relation.get("entity1", entities[0] if entities else "")
                entity2 = relation.get("entity2", entities[1] if len(entities) > 1 else "")
                relation_key = (entity1, entity2)
                if relation_key in relation_id_to_original:
                    original_relation = relation_id_to_original[relation_key]

        # Determine if this is a hyperedge (multi-entity relation)
        entity_count = len(entities) if entities else relation.get("entity_count", 0)
        is_hyperedge = entity_count > 2

        if original_relation:
            # Use original database data and always normalize to hyperedge format.
            orig_entities = original_relation.get("entities", [])
            if isinstance(orig_entities, str):
                orig_entities = [e.strip() for e in orig_entities.split(GRAPH_FIELD_SEP) if e.strip()]
            if not orig_entities:
                src_id = original_relation.get("src_id", "")
                tgt_id = original_relation.get("tgt_id", "")
                orig_entities = [e for e in [src_id, tgt_id] if e]

            rel_fmt = {
                "entities": orig_entities,
                "entity_count": len(orig_entities),
                ...
                "file_path": original_relation.get("file_path", ""),
                "created_at": original_relation.get("created_at", ""),
            }
            if relation.get("rerank_score") is not None:
                try:
                    rel_fmt["rerank_score"] = float(relation["rerank_score"])
                except (TypeError, ValueError):
                    pass
            if relation.get("entity_iou") is not None:
                try:
                    rel_fmt["entity_iou"] = float(relation["entity_iou"])
                except (TypeError, ValueError):
                    pass
            rq = relation.get("rerank_query")
            if rq is not None and str(rq).strip():
                rel_fmt["rerank_query"] = str(rq).strip()
            rqs = relation.get("rerank_queries")
            if isinstance(rqs, list) and rqs:
                rel_fmt["rerank_queries"] = [
                    str(x) for x in rqs if x is not None and str(x).strip()
                ]
            _attach_evidence_catalog_fields(rel_fmt, relation)
            formatted_relationships.append(rel_fmt)
        else:
            # Fallback to LLM context data and always normalize to hyperedge format.
            fallback_entities = entities[:] if entities else []
            if not fallback_entities:
                entity1 = relation.get("entity1", "")
                entity2 = relation.get("entity2", "")
                fallback_entities = [e for e in [entity1, entity2] if e]
    ...
    for i, chunk in enumerate(chunks):
        ...
        if chunk.get("rerank_score") is not None:
            try:
                chunk_data["rerank_score"] = float(chunk["rerank_score"])
            except (TypeError, ValueError):
                pass
        crq = chunk.get("rerank_query")
        if crq is not None and str(crq).strip():
            chunk_data["rerank_query"] = str(crq).strip()
        crqs = chunk.get("rerank_queries")
        if isinstance(crqs, list) and crqs:
            chunk_data["rerank_queries"] = [
                str(x) for x in crqs if x is not None and str(x).strip()
            ]
        _attach_evidence_catalog_fields(chunk_data, chunk)
...
# --- Merged graph fields (<SEP> = GRAPH_FIELD_SEP): multi-source entities / relations ---


def split_graph_field_values(value: Optional[str]) -> List[str]:
    """
    Split database fields that concatenate multiple sources with GRAPH_FIELD_SEP (<SEP>).
    If the delimiter is absent, returns a single-element list (when non-empty).
    """
    if value is None:
        return []
    s = str(value).strip()
    if not s:
        return []
    if GRAPH_FIELD_SEP not in s:
        return [s]
    return [p.strip() for p in s.split(GRAPH_FIELD_SEP) if p and str(p).strip()]


def zip_description_file_path_pairs(
    description: Optional[str], file_path: Optional[str]
) -> List[tuple[str, str]]:
    """
    Pair parallel description and file_path segments merged with <SEP>.
    If counts differ, broadcast the shorter side (last value repeated) like merged KG nodes.
    """
    descs = split_graph_field_values(description)
    paths = split_graph_field_values(file_path)
    if not descs and not paths:
        d0 = (description or "").strip()
        p0 = (file_path or "").strip()
        if d0 or p0:
            return [(d0, p0)]
        return []
    if not descs:
    ...
    if len(descs) == len(paths):
        return list(zip(descs, paths))
    if len(descs) < len(paths):
        ext = descs + [descs[-1]] * (len(paths) - len(descs))
        return list(zip(ext, paths))
    ext = paths + [paths[-1]] * (len(descs) - len(paths))
    return list(zip(descs, ext))





def format_entity_text_multi_source_for_rerank(
    entity_name: str,
    entity_type: str,
    description: Optional[str],
    file_path: Optional[str],
) -> str:
    """Readable multi-line text for cross-encoder / rerank when description+path use <SEP>."""
    pairs = zip_description_file_path_pairs(description, file_path)
    parts: List[str] = []
    if entity_name:
        parts.append(entity_name)
    if entity_type and entity_type != "UNKNOWN":
        parts.append(f"[{entity_type}]")
    head = ": ".join(parts) if parts else (entity_name or "")
    if len(pairs) <= 1:
        d = pairs[0][0] if pairs else (description or "")
        if d and d != "UNKNOWN":
            return f"{head}: {d}" if head else d
        return head or ""
    lines = [head] if head else []
    for i, (d, p) in enumerate(pairs, 1):
        tail = p[-72:] if len(p) > 72 else p
        snippet = (d or "")[:500]
        if tail:
            lines.append(f"  [{i}] source={tail} :: {snippet}")
        else:
            lines.append(f"  [{i}] {snippet}")
    return "\n".join(lines)


def format_relation_text_multi_source_for_rerank(
    entities: Any,
    description: Optional[str],
    file_path: Optional[str],
) -> str:
    """Same as entity helper for hyperedge rows with <SEP>-merged provenance."""
    if isinstance(entities, list):
        ent_s = ", ".join(str(e) for e in entities if e)
    else:
        ent_s = str(entities or "")
    pairs = zip_description_file_path_pairs(description, file_path)
    if len(pairs) <= 1:
        d = pairs[0][0] if pairs else (description or "")
        if ent_s and d:
            return f"{ent_s} — {d}"
        return ent_s or d or ""
    lines = [ent_s] if ent_s else []
    for i, (d, p) in enumerate(pairs, 1):
        tail = p[-72:] if len(p) > 72 else p
        snippet = (d or "")[:500]
        if tail:

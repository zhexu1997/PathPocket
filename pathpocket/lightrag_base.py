...
class QueryParam:
    ...
    """Optional text appended to retrieval context before LLM prompt formatting.
    Used for multimodal queries (e.g. Virchow2 similar-image JSON blocks)."""
    ...
    mix_structured_retrieval: bool = True
    """When True with mix mode and structured keywords (_mix_*), use stem-entity relation/chunk
    retrieval and per-option rerank. Default True; set False or MIX_STRUCTURED_RETRIEVAL=false in batch to disable."""

    mix_structured_chunk_rerank_done: bool = False
    """If True, process_chunks_unified skips the initial rerank (chunks already scored per option)."""

    precomputed_keywords_dict: Optional[Dict[str, Any]] = None
    """If set, ``kg_query`` skips LLM keyword extraction and uses this mapping.
    Expected shape: ``{\"_structured_pathology_llm\": {...}}`` (same schema as
    ``PROMPTS[\"keywords_extraction\"]``). Used when structured pathology was
    already extracted elsewhere (e.g. pipeline stage 01) and the RAG engine runs
    with a no-op ``llm_model_func``."""

    progress_callback: Optional[Callable[[str], None]] = None
    """Optional UI progress hook: called with phase names during retrieval / generation."""
...
class DocProcessingStatus:
    ...
    """Additional metadata"""
    multimodal_processed: Optional[bool] = field(default=None, repr=False)
...
class QueryResult:
    ...
    def metadata(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Query metadata including query_mode, structured_retrieval, etc.
        """
        if self.raw_data:

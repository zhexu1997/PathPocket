...
from pathpocket.lightrag_base import BaseVectorStorage
from pathpocket.lightrag_namespace import resolve_vector_store_cosine_threshold
try:
    from nano_vectordb import NanoVectorDB
except ImportError:
    NanoVectorDB = None
    logger.warning("nano_vectordb not installed, NanoVectorDBStorage will not work")
...
class NanoVectorDBStorage(BaseVectorStorage):
    def __post_init__(self):
        ...
        if NanoVectorDB is None:
            raise ImportError(
                "nano_vectordb is not installed. Please install it with: pip install nano-vectordb"
            )

        # Check if existing file has incompatible format (export format without "matrix")
        if os.path.exists(self._client_file_name):
            try:
                import json
                with open(self._client_file_name, 'r', encoding='utf-8') as f:
                    existing_data = json.load(f)
                    # If file has export format {"data": [...], "embedding_dim": ...} without "matrix", 
                    ...
                    if isinstance(existing_data, dict) and "data" in existing_data and "matrix" not in existing_data:
                        logger.warning(f"Existing vdb file {self._client_file_name} has incompatible format (export format).")
                        logger.info(f"Backing up and removing incompatible file. NanoVectorDB will create a new file.")
                        # Backup old file
                        backup_file = self._client_file_name + ".backup"
                        import shutil
                        shutil.copy2(self._client_file_name, backup_file)
    ...
    async def _get_client(self):
        ...
        async with self._storage_lock:
            ...
            if self.storage_updated.value:
                ...
                # Check if file has incompatible format before reloading
                if os.path.exists(self._client_file_name):
                    try:
                        import json
                        with open(self._client_file_name, 'r', encoding='utf-8') as f:
                            existing_data = json.load(f)
                            if isinstance(existing_data, dict) and "data" in existing_data and "matrix" not in existing_data:
                                logger.warning(f"Reloading: vdb file {self._client_file_name} has incompatible format.")
                                logger.info(f"Backing up and removing incompatible file.")
                                backup_file = self._client_file_name + ".backup"
                                import shutil
                                shutil.copy2(self._client_file_name, backup_file)
                                os.remove(self._client_file_name)
                    except Exception as e:
                        logger.warning(f"Could not check vdb file format during reload: {e}")
    ...
    async def index_done_callback(self) -> bool:
        ...
        async with self._storage_lock:
            ...
            if self.storage_updated.value:
                ...
                if NanoVectorDB is None:
                    raise ImportError("nano_vectordb is not installed")
                self._client = NanoVectorDB(
                    self.embedding_func.embedding_dim,
                    storage_file=self._client_file_name,
                )
